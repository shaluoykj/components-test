<template>
	<div v-if="showSelect" class="pop-container">
		<!--遮罩-->
		<div class="bg-mask" @touchmove.prevent @click="submit(false)"></div>
		<!--选项-->
		<div class="pop-main">
			<div class="pop-header" @touchmove.prevent>
				<span>{{title}}</span>
				<div class="close-icon" @click="submit(false)"></div>
			</div>
			<div class="pop-content">
				<div class="pop-content-body">
					<div v-for="(item,index) in showList" :key="index" :class="{'active-select':(multi&&item.selected)||(!multi&&singleSelectKey===item.key)}" @click="clickItem(item)">
						{{item.name?item.name:item.value}}
					</div>
				</div>
			</div>
			<div class="confirm-btn" @touchmove.prevent>
				<div :class="{'btnChecked':true,'unbtnChecked':selectedList.length===0}" @click="submit(true)">
					确定
				</div>
			</div>
		</div>
	...
	.pop-main {
		...
		.pop-header {
			...
			.close-icon {
				...
				height: 60px;
				width: 57px;
				background: url(../assets/close.png) no-repeat center center / 21px 21px;
			}
		}
		.pop-content {
			flex: 1;
			width: 100%;
			padding: 0px 11.5px 25.5px 11.5px;
			overflow-y: scroll;
			overflow-x: hidden;
